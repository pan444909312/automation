...
public class FileUtils {
    ...
    public static String readTextFormFile(String fileCompletePath) {
        ...
        try {
            var bufferedReader = new BufferedReader(new FileReader(new File(fileCompletePath)));
            var str = "";
    ...
    public static String getFilePathFormResource(String filePath) {
        try {
            return Objects.requireNonNull(FileUtils.class.getClassLoader().getResource(filePath)).getPath();
    ...
    public static void writeTextToFile(String text, String filePath) {
        ...
        Writer writer = null;
        try {
            writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(sourceFile), StandardCharsets.UTF_8));
    ...
    public static String doCopy(String sourceFilePath, String targetFilePath) throws IOException {
        if (!sourceFilePath.equals(targetFilePath)) {
            var inFile = new File(sourceFilePath);
            var outFile = new File(targetFilePath);
            //判断父路径是否存在，不存在创建一个
    ...
    public static boolean doCopyDir(String source_dir, String target_dir) throws IOException {
        //顶层目录
        var source_file = new File(source_dir);
        var target_file = new File(target_dir);
        //复制目录
            ...
            if (sourceDir != null) {
                for (var i = 0; i < sourceDir.length; i++) {
                    if (null != sourceDir[i] && sourceDir[i].isFile()) {
                        //拷贝文件
                        var targetFile = new File(target_dir + File.separator + sourceDir[i].getName());
                        try {
    ...
    public static String copyFile(String tempPath, String dir, String uploadDir) throws Exception {
        ...
        if (tempPath.contains("/temp/")) {
            var tempFile = uploadDir + "//temp//" + fileName;
            var targetFile = uploadDir + "//" + dir + "//" + fileName;
