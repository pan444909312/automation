...
//            throw new IllegalArgumentException("请求头中缺少 Content-Type 字段");
    ...
    public static JSONObject categoryGetShopVOByShopId(String shopId, String mobilePhone, String password, String testgroup, String longitude, String latitude, String apptypeid, String language, Integer isNeedMarketCategory, Integer marketCategoryId, String zipcode) {
        ...
        String method = "POST";
        String headers = "module/shopList/recall/category/request/headers.json";
        String body = "module/shopList/recall/category/request/body.json";
     ...
     * @param apptypeid            站点，可为空
     * @param language             语言，可为空
     * @param channelGroupId       分类id，可为空
     * @param channelId            专题id，可为空
     * @param filtering            过滤开关，可为空
     * @return 店铺在品类专题店铺流的VO信息, 当品类专题店铺流不存在店铺时返回null
     */
    public static JSONObject channelPfGetShopVOByShopId(Integer shopId, Integer channelGroupId,Integer channelId, Integer filtering, String mobilePhone, String password, String testgroup, String longitude, String latitude, String apptypeid, String language) {
        ...
        while (pageNo <= maxPage) {
            try {
                ...
                if (channelId != null) {
                    requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.channelId", channelId);
                }
                if (filtering != null) {
                    requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.filtering", filtering);
                }
                ...
                for (int i = 0; i < shopList.size(); i++) {
                    JSONObject shop = shopList.getJSONObject(i);
                    if (shopId.equals(shop.getInteger("shopId"))) {
                        return shop;
                    }
                }
