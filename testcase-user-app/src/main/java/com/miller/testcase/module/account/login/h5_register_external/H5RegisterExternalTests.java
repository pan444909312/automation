 ...
 */
@Scenario(
        scenarioID = "01K596AAX24KJXRHQ7H51VEVNM", // 自动生成，不要修改
        scenarioName = "H5-新人注册：新账户注册成功",
        author = "yancancan@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("H5-新人注册：新账户注册成功")
public class H5RegisterExternalTests {
    ...
    void shouldSuccess() {
        ...
        String telephone = getPhoneNumber(mobile);
        System.out.println("获取到的手机号: " + telephone);
        String user_id =PandaTestDBHelpful.executeSelectOneSql("select user_id from user where user_name = ?",telephone).get("user_id").toString();
        System.out.println("获取到的用户id: " + user_id);
        TestCaseHelpful.assertThat(PandaTestDBHelpful.executeSelectListSql("select * from user where user_id=?", user_id).get(0).get("register_source")).isEqualTo(null);
