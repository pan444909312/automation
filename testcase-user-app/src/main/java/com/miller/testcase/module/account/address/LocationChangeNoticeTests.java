...
@Scenario(
        scenarioID = "01JXW1RVZY7XT88QH4D3G2Z5H6",
        scenarioName = "用户位置变更通知",
        author = "huyang@hungrypandagroup.com",
        developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@DisplayName("用户位置变更通知")
public class LocationChangeNoticeTests {
    // 接口请求的 path
    String uri = TestcaseConfig.HOST_APP + "/api/app/user/location/change/notice";
    // 请求方式
    String method = "POST";
    // 请求头
    String headers = "module/headers.json";
    // 请求体。如果没有传 null 即可（body = null）。比如 GET 请求
    String body = "module/account/address/request/LocationChangeNoticeReq.json";
    // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 paras 参数
    String params = null;
    // 断言
    String assert1 = "module/account/address/response/LocationChangeNoticeResp.json";
