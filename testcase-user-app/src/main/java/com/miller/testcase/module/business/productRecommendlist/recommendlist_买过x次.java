...
@Scenario(
        scenarioID = "01K796GHDAD61Y1T9EHQ52JXZK",
        scenarioName = "商品详情推荐列表-商品标签：买过x次",
        author = "yaoqianhu@hungrypandagroup.com",
        developmentTime = 15, maintenanceTime = 0, manualTestTime = 10)
@DisplayName("商品详情推荐列表-商品标签：买过x次")
public class recommendlist_买过x次 {
    // 接口请求的 path
    ...
    public void getMenuListSuccessfully() {
        ...
        Object result = TestCaseHelpful.extractValue(responseBody,"$.result.recommendProductList[*].systemMarketingTags[*][?(@.tagType==3)]");
        ...
        TestCaseHelpful.assertThatJson(result).when(Option.IGNORING_EXTRA_FIELDS,Option.IGNORING_EXTRA_ARRAY_ITEMS,Option.IGNORING_ARRAY_ORDER).isEqualTo(expectedStr2);
