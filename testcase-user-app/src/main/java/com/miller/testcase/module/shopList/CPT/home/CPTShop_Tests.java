 ...
 */
@Scenario(
        scenarioID = "01K1YY3XXZFCGAR5ASAJMBEZ3Y", // 自动生成，不要修改
        scenarioName = "中文版_店铺流_广告_首页_CPT-召回：满足CPT",
        author = "huyang@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 30, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("中文版_店铺流_广告_首页_CPT-召回：满足CPT")
public class CPTShop_Tests {
...
//        需要修改实验时，传入新的全量实验
        ...
        var responseShopBody = TestCaseHelpful.getShopVOByShopId("305457843", "19157826865", "123456a", testGroupNew, null,null,null);
        // 检查响应体不为空
...
//        assertThat(responseShopBody).isNull();
        ...
        Assertions.assertNotNull(adPosition, "adPosition 不应为空");
        assertThat(adPosition).isEqualTo(1);
        // 获取并校验 saType
