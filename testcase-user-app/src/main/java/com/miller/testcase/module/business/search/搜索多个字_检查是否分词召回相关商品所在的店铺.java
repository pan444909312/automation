...
@Scenario(scenarioID = "01K5EBW00FVVAD8HFW0W0ZPPRK",
        scenarioName = "搜索多个字_检查是否分词召回相关商品所在的店铺",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@DisplayName("/api/user/v2/search")
public class 搜索多个字_检查是否分词召回相关商品所在的店铺 {
    private static final String uri = TestcaseConfig.HOST_APP + "/api/user/v2/search";

    private String shopId = "74314835";
    private String productId = "82551582";

    @DisplayName("搜索多个字_检查是否分词召回相关商品所在的店铺")
    @Test
    void shouldReturnSuccessfully() {
        ...
        TestCaseHelpful.assertThat(result2.toString().contains("奶茶")).isEqualTo(false);
        TestCaseHelpful.assertThat(result3.toString().contains("奶茶")).isEqualTo(true);
