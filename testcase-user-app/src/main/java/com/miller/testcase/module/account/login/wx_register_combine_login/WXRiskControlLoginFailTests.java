package com.miller.testcase.module.account.login.wx_register_combine_login;
...
/**
 * wx register combine login
 *
 * @author yancancan
 * @version 2.0
 * @since 2025/08/19 14:55:40
 */
@Scenario(
        scenarioID = "01K44MH6AQSSDE6X8FP9ZEG9AW", // 自动生成，不要修改
        scenarioName = "微信小程序渠道：新账户注册失败（极验设备注册账号已达上限）",
        author = "yancancan@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("微信小程序渠道：新账户注册失败（极验设备注册账号已达上限）")
public class WXRiskControlLoginFailTests {
    ...
    void shouldSuccess() {
        ...
        String headers = "module/account/login/wx_register_combine_login/request/headers.json";
        // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 params 参数
        String params = null;
        // 请求体。如果没有传 null 即可（body = null）。比如 GET 请求可能没有请求体。作用同请求头
        String body = "module/account/login/wx_register_combine_login/request/body.json";
        // 断言。默认从resources目录下读取文件。下面的代码表示从 resource 的 module/xxx/response/assert_full_field.json 读取文件内容作为断言
        String assertFullField = "module/account/login/wx_register_combine_login/response/assert_full_field.json";
        ...
        TestCaseHelpful.assertThatJson(responseBody).when(Option.IGNORING_EXTRA_FIELDS).inPath("$.resultCode").isEqualTo(500);
        TestCaseHelpful.assertThatJson(responseBody).when(Option.IGNORING_EXTRA_FIELDS).inPath("$.error").isEqualTo("该设备登录账号已达上限无法登录/注册");
