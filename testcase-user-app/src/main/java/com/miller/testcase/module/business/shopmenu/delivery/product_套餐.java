...
@Scenario(
        scenarioID = "01K4BSG3429G3PZ80EQF5MR1CZ",
        scenarioName = "商卡-商品信息-是套餐商品",
        author = "yaoqianhu@hungrypandagroup.com",
        developmentTime = 15, maintenanceTime = 0, manualTestTime = 10)
@DisplayName("商卡-商品信息-是套餐商品")
public class product_套餐 {
    // 接口请求的 path
    ...
    String assert1 = "module/home/shop/response/product_套餐.json";
    @DisplayName("正向流程")
    @Test
    public void getMenuListSuccessfully() {
        ...
        Object result = TestCaseHelpful.extractValue(responseBody,"$.result.menuList[0].subMenuList[*].productList[?(@.productId==82457768)]");        // 步骤4: 断言响应结果，直接拷贝抓包响应结果作为断言。基本固定写法，不需要修改
