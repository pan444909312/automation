...
@Scenario(
        scenarioID = "01K797KG9Z8Z8SR7HBXY3A3P1G",
        scenarioName = "商品详情推荐列表-商品标签：最近有x个回头客",
        author = "yaoqianhu@hungrypandagroup.com",
        developmentTime = 15, maintenanceTime = 0, manualTestTime = 10)
@DisplayName("商品详情推荐列表-商品标签：最近有x个回头客")
public class recommendlist_最近有x个回头客 {
    // 接口请求的 path
    ...
    public void getMenuListSuccessfully() {
        ...
        Object result = TestCaseHelpful.extractValue(responseBody,"$.result.recommendProductList[*].systemMarketingTags[*][?(@.tagType==5)]");
