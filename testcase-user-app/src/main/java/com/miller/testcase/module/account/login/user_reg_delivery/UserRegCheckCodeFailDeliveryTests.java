 ...
 */
@Scenario(
        scenarioID = "01K5961XM76CX83C2Z4HQX6A2P", // 自动生成，不要修改
        scenarioName = "骑手拉新-注册：新账户注册失败（验证码错误）",
        author = "yancancan@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("骑手拉新-注册：新账户注册失败（验证码错误）")
public class UserRegCheckCodeFailDeliveryTests {
    ...
    void shouldSuccess() {
        ...
        requestBody=TestCaseHelpful.updateJsonValue(requestBody, "$.pd.captcha", "111111");
        ...
        TestCaseHelpful.assertThatJson(responseBody).when(Option.IGNORING_EXTRA_FIELDS).inPath("$.resultCode").isEqualTo(2011);
        TestCaseHelpful.assertThatJson(responseBody).when(Option.IGNORING_EXTRA_FIELDS).inPath("$.error").isEqualTo("验证码错误，请重新输入");
