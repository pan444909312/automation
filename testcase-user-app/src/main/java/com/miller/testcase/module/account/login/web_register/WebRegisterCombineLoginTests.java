package com.miller.testcase.module.account.login.web_register;
...
/**
 * web register combine login
 *
 * @author yancancan
 * @version 2.0
 * @since 2025/08/19 14:07:08
 */
@Scenario(
        scenarioID = "01K30F0G9R2F0AP8CKT58F9RGE", // 自动生成，不要修改
        scenarioName = "\"WEB站渠道：新账户注册成功",
        author = "yancancan@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("WEB站渠道：新账户注册成功")
public class WebRegisterCombineLoginTests {
    ...
    static void beforeAll(){
        ...
        String tel="15900000007";        String telephone = getPhoneNumber(tel);
    ...
    @Test
    void shouldSuccess() {
        String tel="15900000007";
        // TestcaseConfig.HOST 是接口的请求域名。 后面的 + "是接口的请求路径"
        String uri = TestcaseConfig.HOST_APP + "/api/user/combine/login";
        // 接口请求方式。如： GET、POST、PUT、DELETE
        String method = "GET";
        // 请求头。默认从 resources 目录下读取文件。
        String headers = "module/account/login/web_register_combine_login/request/headers.json";
        // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 params 参数
        String params = null;
        // 请求体。如果没有传 null 即可（body = null）。比如 GET 请求可能没有请求体。作用同请求头
        String body = "module/account/login/web_register_combine_login/request/body.json";
        // 断言。默认从resources目录下读取文件。下面的代码表示从 resource 的 module/xxx/response/assert_full_field.json 读取文件内容作为断言
        String assertFullField = "module/account/login/web_register_combine_login/response/assert_full_field.json";
        ...
        var verification=TestCaseHelpful.getVerificationCode("15900000007");
        ...
        //获取新注册成功的uid
        String telephone = getPhoneNumber("15900000007");
        System.out.println("获取到的手机号: " + telephone);
        String user_id = PandaTestDBHelpful.executeSelectOneSql("select user_id from user where user_name = ?",telephone).get("user_id").toString();
        System.out.println("获取到的用户id: " + user_id);
        //校验表数据
