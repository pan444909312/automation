package com.miller.testcase.module.account.login.wx_combine_login;
 ...
 */
@Scenario(
        scenarioID = "01K4434X5VAVRJHSWK57EF9S62", // 自动生成，不要修改
        scenarioName = "微信小程序渠道：老用户登录失败（极验设备登录账号已达上限）",
        author = "yancancan@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("微信小程序渠道：老用户登录失败（极验设备登录账号已达上限）")
public class WXRiskControlLoginFailTests {
    ...
    void shouldSuccess() {
        ...
        String headers = "module/account/login/wx_combine_login/request/headers.json";
        // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 params 参数
        String params = null;
        // 请求体。如果没有传 null 即可（body = null）。比如 GET 请求可能没有请求体。作用同请求头
        String body = "module/account/login/wx_combine_login/request/body.json";
        // 断言。默认从resources目录下读取文件。下面的代码表示从 resource 的 module/xxx/response/assert_full_field.json 读取文件内容作为断言
        String assertFullField = "module/account/login/wx_combine_login/response/assert_full_field.json";
