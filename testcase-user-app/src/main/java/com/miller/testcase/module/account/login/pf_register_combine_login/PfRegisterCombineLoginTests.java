package com.miller.testcase.module.account.login.pf_register_combine_login;
...
/**
 * pf register combine login
 *
 * @author yancancan
 * @version 2.0
 * @since 2025/08/19 14:47:42
 */
@Scenario(
        scenarioID = "01K30HAS64CV5AG85QSME2ZB30", // 自动生成，不要修改
        scenarioName = "pf register combine login",
        author = "yancancan@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("PF套壳-注册：新账户注册成功")
public class PfRegisterCombineLoginTests {
    ...
    @Test
    void shouldSuccess() {
        String tel="15900000009";
        // TestcaseConfig.HOST 是接口的请求域名。 后面的 + "是接口的请求路径"
        String uri = TestcaseConfig.HOST_APP + "/api/user/combine/login";
        // 接口请求方式。如： GET、POST、PUT、DELETE
        String method = "POST";
        // 请求头。默认从 resources 目录下读取文件。
        String headers = "module/account/login/pf_register_combine_login/request/headers.json";
        // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 params 参数
        String params = null;
        // 请求体。如果没有传 null 即可（body = null）。比如 GET 请求可能没有请求体。作用同请求头
        String body = "module/account/login/pf_register_combine_login/request/body.json";
        // 断言。默认从resources目录下读取文件。下面的代码表示从 resource 的 module/xxx/response/assert_full_field.json 读取文件内容作为断言
        String assertFullField = "module//account/login/pf_register_combine_login/response/assert_full_field.json";
        ...
        TestCaseHelpful.assertThat(PandaTestDBHelpful.executeSelectListSql("select * from user where user_id=?", user_id).get(0).get("register_source")).isEqualTo(10);
//      //清除已注册用户数据
