package com.miller.testcase.module.account.login.user_reg_delivery;
 ...
 * @author yancancan
 * @version 2.0
 * @since 2025/09/16 13:39:42
 */
@Scenario(
        scenarioID = "01K595JFW5J2PXS03JJ091Y9D8", // 自动生成，不要修改
        scenarioName = "骑手拉新-注册：新账户注册失败（验证码区号不匹配）",
        author = "yancancan@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("骑手拉新-注册：新账户注册失败（验证码区号不匹配）")
public class UserRegAreaCodeFailDeliveryTests {

    @BeforeAll
    static void beforeAll(){
        // 所有 @Test 方法执行之前会执行  @BeforeAll 注解的方法, 这里的代码当前测试类期间只会执行一次
        // 你可以在这里执行前置的操作，比如: SQL 初始化用例的前置条件
        String telephone = getPhoneNumber("16729398999");
    ...
    @Test
    void shouldSuccess() {
        String mobile="16729398999";
        // TestcaseConfig.HOST 是接口的请求域名。 后面的 + "是接口的请求路径"
        String uri = TestcaseConfig.HOST_APP + "/api/user/reg";
        // 接口请求方式。如： GET、POST、PUT、DELETE
        String method = "GET";
        // 请求头。默认从 resources 目录下读取文件。
        String headers = "module/account/login/user_reg_delivery/request/headers.json";
        // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 params 参数
        String params = null;
        // 请求体。如果没有传 null 即可（body = null）。比如 GET 请求可能没有请求体。作用同请求头
        String body = "module/account/login/user_reg_delivery/request/body.json";
        // 断言。默认从resources目录下读取文件。下面的代码表示从 resource 的 module/xxx/response/assert_full_field.json 读取文件内容作为断言
        String assertFullField = "module/account/login/user_reg_delivery/response/assert_full_field.json";
