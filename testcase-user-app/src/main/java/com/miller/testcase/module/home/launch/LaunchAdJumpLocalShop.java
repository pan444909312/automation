...
@Scenario(scenarioID = "01K44EK18SVMS7NDFMH15TSC9Z",
        scenarioName = "拉取启动页广告-跳转团购店铺",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@DisplayName("/api/user/v1/launchAd/pull")
public class LaunchAdJumpLocalShop {
    private static final String uri = TestcaseConfig.HOST_APP + "/api/user/v1/launchAd/pull";

    @BeforeAll
    void beforeAll() {
        LaunchAdUtils.initCityLaunchAd("九江市");
        PandaTestDBHelpful.executeInsertOrUpdateOrDelete("update hp_launch_ad set state = 1 where la_id = 276");
    }

    @AfterAll
    void afterAll() {
        PandaTestDBHelpful.executeInsertOrUpdateOrDelete("update hp_launch_ad set state = 0 where la_id = 276");
    }

    @DisplayName("拉取启动页广告-跳转团购店铺")
    @Test
    void shouldReturnSuccessfully() {
        ...
        TestCaseHelpful.assertThat(jumpUrl.toString()).isEqualTo("hptakeout://module=deeplink&action=openPage&params=eyJ1cmwiOiJocHRha2VvdXQ6Ly92b3VjaGVyL3Nob3BEZXRhaWw/c2hvcElkPTE5NTQyMDAzNyJ9&callback=CALLBACK_NAME&code=7708efec67f7422");
        TestCaseHelpful.assertThat((int)jumpType).isEqualTo(4);
