...
@Scenario(scenarioID = "01K44FA2F8H4C2ECV7YWMTZSKY",
        scenarioName = "拉取启动页广告-有效期过期会正常返回（客户端处理）",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@DisplayName("/api/user/v1/launchAd/pull")
public class LaunchAdPeriodHasExpired {
    private static final String uri = TestcaseConfig.HOST_APP + "/api/user/v1/launchAd/pull";

    @BeforeAll
    void beforeAll() {
        LaunchAdUtils.initCityLaunchAd("九江市");
        PandaTestDBHelpful.executeInsertOrUpdateOrDelete("update hp_launch_ad set state = 1 where la_id = 282");
    }

    @AfterAll
    void afterAll() {
        PandaTestDBHelpful.executeInsertOrUpdateOrDelete("update hp_launch_ad set state = 0 where la_id = 282");
    }

    @DisplayName("拉取启动页广告-有效期过期会正常返回（客户端处理）")
    @Test
