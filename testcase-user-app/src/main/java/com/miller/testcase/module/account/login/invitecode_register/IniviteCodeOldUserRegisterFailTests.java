...
/**
 * inivitecode Old user register fail
 *
 * @author yancancan
 * @version 2.0
 * @since 2025/09/02 15:55:07
 */
@Scenario(
        scenarioID = "01K44PT1PFQDM5MAQG4X546MDV", // 自动生成，不要修改
...
public class IniviteCodeOldUserRegisterFailTests {
...
//        // 清除已注册用户数据
    ...
    void shouldSuccess() {
        ...
        String uri = TestcaseConfig.HOST_APP + "/api/app/user/check/invite";
        // 接口请求方式。如： GET、POST、PUT、DELETE
        String method = "POST";
        // 请求头。默认从 resources 目录下读取文件。
        String headers = "module/account/login/inivitecode_old_user_register_fail/request/headers.json";
        // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 params 参数
        String params = null;
        // 请求体。如果没有传 null 即可（body = null）。比如 GET 请求可能没有请求体。作用同请求头
        String body = "module/account/login/inivitecode_old_user_register_fail/request/body.json";
        // 断言。默认从resources目录下读取文件。下面的代码表示从 resource 的 module/xxx/response/assert_full_field.json 读取文件内容作为断言
        String assertFullField = "module/account/login/inivitecode_old_user_register_fail/response/assert_full_field.json";
        ...
        TestCaseHelpful.assertThatJson(responseBody).when(Option.IGNORING_EXTRA_FIELDS).inPath("$.resultCode").isEqualTo(1000);
        TestCaseHelpful.assertThatJson(responseBody).when(Option.IGNORING_EXTRA_FIELDS).inPath("$.result.isExist").isEqualTo(1);
