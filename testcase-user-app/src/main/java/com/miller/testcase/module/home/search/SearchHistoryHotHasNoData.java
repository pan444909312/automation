...
public class SearchHistoryHotHasNoData {
    ...
    void shouldReturnSuccessfully() {
        List<Map<String, Object>> sqlList = PandaTestDBHelpful.executeSelectListSql("SELECT sh.search_hot_id FROM search_hot sh LEFT JOIN search_hot_link shl on sh.search_hot_id = shl.search_hot_id WHERE sh.city is null and sh.start_time != 0 and shl.city in ('全部城市','九江市') and sh.status = 1");
