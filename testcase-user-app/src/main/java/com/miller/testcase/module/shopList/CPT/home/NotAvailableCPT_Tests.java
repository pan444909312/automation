package com.miller.testcase.module.shopList.CPT.home;
 ...
 * @author HuYang
 * @version 2.0
 * @since 2025/08/06 11:41:40
 */
@Scenario(
        scenarioID = "01K1Z5D1SK0ENEVZQ8AMBA8FM7", // 自动生成，不要修改
        scenarioName = "中文版_店铺流_广告_首页_CPT-召回：未上架CPT - 不召回",
        author = "huyang@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 30, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("中文版_店铺流_广告_首页_CPT-召回：未上架CPT - 非CPT")
public class NotAvailableCPT_Tests {
...
//        需要修改实验时，传入新的全量实验
        ...
        var responseShopBody = TestCaseHelpful.getShopVOByShopId("244938161", "19157826865", "123456a", testGroupNew, null,null,null);
        // 检查响应体不为空
...
//        assertThat(responseShopBody).isNull();
        ...
        // 获取并校验 adPosition
        Integer adPosition = responseShopBody.getInteger("adPosition");
        assertThat(adPosition).isNull();
        // 获取并校验 saType
        Integer saType = responseShopBody.getInteger("saType");
        assertThat(saType).isNull();
        // 获取并校验 saId
        Integer saId = responseShopBody.getInteger("saId");
        assertThat(saId).isNull();
    }
} 