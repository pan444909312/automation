package com.miller.userapp.moduleEnAu.shop.card.version3.userPack.promotion.godCouponV3;
...
@Scenario(scenarioID = "01K7EE2BWXF6RC5CE3HNXK95PX", scenarioName = "普通店铺自取商卡-SKYX01_优惠标签_神券_自取频道-商卡二期-SKYX实验组：神券标签41-已膨胀至X｜店铺未加码",
...
public class ShopShouldHasGodCouponExpandedTests {
    ...
    @ParameterizedTest
    @DisplayName("普通店铺自取商卡-SKYX01_优惠标签_神券_自取频道-商卡二期-SKYX实验组：神券标签41-已膨胀至X｜店铺未加码")
     void couponGodDsicount(ShopListRequestDTO shopListRequestDTO) {
          ShopListResponseDTO shopList = ShopListFlow.getShopListByShopId(shopListRequestDTO,shopId);
          
          if (shopList == null||shopList.getResult() == null) {
              System.out.println("01K7EE2BWXF6RC5CE3HNXK95PX错误：shopList.getResult() 为 null，resultCode: " + shopList.getResultCode());
              return;
          }
          
          if (shopList.getResult().getShopList() == null || shopList.getResult().getShopList().isEmpty()) {
              System.out.println("01K7EE2BWXF6RC5CE3HNXK95PX错误：shopList.getResult().getShopList() 为 null 或为空");
              return;
          }
          ...
          if (shopIndexOptional.isEmpty()) {
              System.out.println("01K7EE2BWXF6RC5CE3HNXK95PX错误：未找到 shopId 为 " + shopId + " 的店铺");
              return;
          }
          ...
          if (shopIndexVO.getShopPromoteList() == null || shopIndexVO.getShopPromoteList().isEmpty()) {
              System.out.println("01K7EE2BWXF6RC5CE3HNXK95PX错误：shopIndexVO.getShopPromoteList() 为 null 或为空");
              return;
          }
          System.out.println("01K7EE2BWXF6RC5CE3HNXK95PX营销信息：" + shopIndexVO.getShopPromoteList());
          ...
          if (shopPromoteOptional.isEmpty()) {
              System.out.println("01K7EE2BWXF6RC5CE3HNXK95PX错误：未找到类型为 SUPER_COUPON 的优惠标签");
              return;
          }
