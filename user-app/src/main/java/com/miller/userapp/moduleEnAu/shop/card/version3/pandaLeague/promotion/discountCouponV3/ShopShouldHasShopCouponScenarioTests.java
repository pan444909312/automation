package com.miller.userapp.moduleEnAu.shop.card.version3.pandaLeague.promotion.discountCouponV3;
 ...
 * date: 2025/07/26
 */
@Scenario(scenarioID = "01K47416DXFVM27WFYXACDTBZ2", scenarioName = "商卡(中文)_普通店铺配送商卡-熊猫联盟频道_优惠标签_店铺红包熊猫联盟频道-商卡二期-SKYX实验组：店铺红包42",
        author = "yancancan@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 10)
@EnvTag.Test
@DisplayName("商卡(中文)")
public class ShopShouldHasShopCouponScenarioTests {
    ...
    @ParameterizedTest
    @DisplayName("商卡(中文)_普通店铺配送商卡-熊猫联盟频道_优惠标签_店铺红包熊猫联盟频道-商卡二期-SKYX实验组：店铺红包42")
    void shouldShowPandLeagueFullSubCouponLabel(ShopListRequestDTO shopListRequestDTO) {
        ...
        assertThat(shopPromoteList.size()).isEqualTo(4);
        assertThat(shopPromoteList.get(0).getShowContent()).isEqualTo("¥3无门槛");
        assertThat(shopPromoteList.get(1).getShowContent()).isEqualTo("3折无门槛");
        assertThat(shopPromoteList.get(2).getShowContent()).isEqualTo("¥5红包");
        assertThat(shopPromoteList.get(3).getShowContent()).isEqualTo("5折红包");
        assertThat(shopPromoteList.get(0).getTagType()).isEqualTo(1);
        assertThat(shopPromoteList.get(3).getTagType()).isEqualTo(1);
