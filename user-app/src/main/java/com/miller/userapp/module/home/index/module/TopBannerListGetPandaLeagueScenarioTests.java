...
/**
 * @author panjuxiang
 * @since 2024/7/18 17:31
 */
@EnvTag.Test
@TestFramework
@DisplayName("用户-首页资源位-中文版-banner-跳转类型为熊猫联盟-展示")
public class TopBannerListGetPandaLeagueScenarioTests {

    @DisplayName("正常流程_获取跳转类型为熊猫联盟的banner")
    @Test
    public void topBannerListGetPandaLeagueSuccessfully() {
        ...
        SingleModuleVo singleModuleVo = topBannerList.stream()
                .filter(banner -> banner.getIndexModuleDetailId() == Integer.parseInt(new PropertiesUtils().getProperty(this.getClass(),"user.app.index.banner.index.module.detailId.type23")))
                .findFirst()
                .get();
        ...
        assert singleModuleVo.getType() == 23;
        assert singleModuleVo.getUrl().equals(DeepLinkUrlConstant.PANDA_LEAGUE);
    }
}
