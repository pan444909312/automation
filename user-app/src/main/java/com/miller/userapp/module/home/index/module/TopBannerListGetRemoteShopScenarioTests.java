...
/**
 * @author panjuxiang
 * @since 2024/7/18 17:17
 */
@EnvTag.Test
@TestFramework
@DisplayName("用户-首页资源位-中文版-banner-跳转类型为远距离商家-展示")
public class TopBannerListGetRemoteShopScenarioTests {

    @DisplayName("正常流程_获取跳转类型为远距离商家的banner")
    @Test
    public void topBannerListGetRemoteShopSuccessfully() {
        ...
        SingleModuleVo singleModuleVo = indexOperateModuleList.getResult()
                ...
                .getTopBannerList()
                .stream()
                .filter(banner -> banner.getIndexModuleDetailId() == Integer.parseInt(new PropertiesUtils().getProperty(this.getClass(),"user.app.index.banner.index.module.detailId.type5")))
                .findFirst()
                .get();
        ...
        try {
            ...
            assert singleModuleVo.getType() == 5;
            assert UrlUtils.isValidUrl(url,"https://leaflet-f2e-test.hungrypanda.cn/remoteShopList","title");
