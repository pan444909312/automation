...
/**
 * @author panjuxiang
 * @since 2024/7/18 17:26
 */
@EnvTag.Test
@TestFramework
@DisplayName("用户-首页资源位-中文版-banner-跳转类型为超市-展示")
public class TopBannerListGetSupermarketScenarioTests {

    @DisplayName("正常流程_获取跳转类型为超市的banner")
    @Test
    public void topBannerListGetSupermarketSuccessfully() {
        ...
        SingleModuleVo singleModuleVo = topBannerList.stream()
                .filter(banner -> banner.getIndexModuleDetailId() == Integer.parseInt(new PropertiesUtils().getProperty(this.getClass(),"user.app.index.banner.index.module.detailId.type13")))
                .findFirst()
                .get();
        ...
        assert singleModuleVo.getType() == 13;
        assert singleModuleVo.getUrl().equals(DeepLinkUrlConstant.SUPERMARKET);
    }
}
