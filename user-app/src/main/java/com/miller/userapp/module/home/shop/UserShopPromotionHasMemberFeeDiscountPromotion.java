...
@Scenario(scenarioID = "01JVKR6DPY3AY792BTB6AE5DCT",
        scenarioName = "店铺的优惠信息包含会员运费优惠",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@EnvTag.Test
@DisplayName("/api/app/user/shop/promotion")
public class UserShopPromotionHasMemberFeeDiscountPromotion {
    ...
    void testCase(ShopBaseReqDTO shopBaseReqDTO) {
        ...
        String shopNewUserPromotion =  JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 10)]").toString();
        Integer threshold = (Integer) JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 10)].discountInfoList[0].threshold");
        Integer maxValue = (Integer) JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 10)].discountInfoList[0].maxValue");


        assertThat(resultCode).isEqualTo(1000);
        assertThat(shopNewUserPromotion).isNotNull();
        assertThat(threshold).isEqualTo(100);
        assertThat(maxValue).isEqualTo(600);
