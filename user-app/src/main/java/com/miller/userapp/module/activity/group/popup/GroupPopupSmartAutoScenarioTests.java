...
@EnvTag.Test
@TestFramework
@Scenario(scenarioID = "01JV6RCKS5ZXCGMR7CTCMFTFPC", scenarioName = "智能营销链路-天降弹窗-（基础金额开关开+直塞模式+3张神券）"
        , author = "yancancan@hungrypandagroup.com", developmentTime = 120, maintenanceTime = 0, manualTestTime = 15)
public class GroupPopupSmartAutoScenarioTests {

//    智能营销3张券基础配置
private static final List<GroupPopupSmartAutoScenarioTests.RedPacketConfig> RED_PACKET_CONFIGS = Arrays.asList(
        new GroupPopupSmartAutoScenarioTests.RedPacketConfig(888893278L, 100, 2700),
        new GroupPopupSmartAutoScenarioTests.RedPacketConfig(888893280L, 100, 3200),
        new GroupPopupSmartAutoScenarioTests.RedPacketConfig(888893276L, 300, 2200)
);
    @BeforeAll
...
//        清除用户已领取的天降数据
        ...
        for (GroupPopupSmartAutoScenarioTests.RedPacketConfig redPacketConfig : RED_PACKET_CONFIGS) {
            new UserCdkeyDeleteSql().deleteUserCdkey(userId, redPacketConfig.getId());
        }
//        修改九江市领取模式为直塞
        new AcitivityCityUpdateSql().setCityRuleAuto("九江市",0);
    }
    @DisplayName("智能营销链路-天降弹窗-（基础金额开关开+直塞模式+3张神券）")
...
//        获取天降结果
    ...
    private void verifyRedPacketOrder(List<RedPacketGroupPopupInfoVO.RedPacketSimpleVO> redPackets) {
        assertThat(redPackets.get(0).getRedPacketId()).isEqualTo(888893276L);
        assertThat(redPackets.get(1).getRedPacketId()).isEqualTo(888893278L);
        assertThat(redPackets.get(2).getRedPacketId()).isEqualTo(888893280L);
    }
    ...
    private void verifyUserReceiveStatus() {
        ...
        for (RedPacketConfig config : RED_PACKET_CONFIGS) {
            ...
            assertThat(cdKey.getRedPacketScopeType())
    ...
    private void verifyRedPacketConfig(
            ...
            GroupPopupSmartAutoScenarioTests.RedPacketConfig config
    ) {
