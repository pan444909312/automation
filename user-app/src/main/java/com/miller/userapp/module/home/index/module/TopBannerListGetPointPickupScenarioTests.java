...
/**
 * @author panjuxiang
 * @since 2024/7/18 17:13
 */
@EnvTag.Test
@TestFramework
@DisplayName("用户-首页资源位-中文版-banner-跳转类型为定点取餐-展示")
@Log4j2
public class TopBannerListGetPointPickupScenarioTests {

    @DisplayName("正常流程_获取跳转类型为定点取餐的banner")
    @Test
    public void topBannerListGetPointPickupSuccessfully() {
        ...
        SingleModuleVo singleModuleVo = topBannerList.stream()
                .filter(banner -> banner.getIndexModuleDetailId() == Integer.parseInt(new PropertiesUtils().getProperty(this.getClass(),"user.app.index.banner.index.module.detailId.type4")))
                .findFirst()
                .get();
        ...
        assert singleModuleVo.getType() == 4;
        assert singleModuleVo.getUrl().equals(DeepLinkUrlConstant.POINT_PICKUP);
    }
}
