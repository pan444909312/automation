...
@Scenario(scenarioID = "01JVKR6DPY3AY792BTB6AE5DCQ",
        scenarioName = "店铺的优惠信息包含新客爆品优惠",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@EnvTag.Test
@DisplayName("/api/app/user/shop/promotion")
public class UserShopPromotionHasShopNewUserProductPromotion {
    ...
    void testCase(ShopBaseReqDTO shopBaseReqDTO) {
        ...
        String shopNewUserPromotion =  JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 601)]").toString();
        Integer value = (Integer) JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 601)].discountInfoList[0].value");


        assertThat(resultCode).isEqualTo(1000);
        assertThat(shopNewUserPromotion).isNotNull();
        assertThat(value).isEqualTo(616);
