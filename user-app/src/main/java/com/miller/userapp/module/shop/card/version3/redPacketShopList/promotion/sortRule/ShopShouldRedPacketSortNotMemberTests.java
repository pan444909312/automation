package com.miller.userapp.module.shop.card.version3.redPacketShopList.promotion.sortRule;
...
@Scenario(scenarioID = "01KE9JG2QWDYJ9ERQ4HSYH8C4Z", scenarioName = "红包适用商家列表-商卡二期-SKYX实验组：红包类型标签-优先级（非会员默认排序）",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 15)
@EnvTag.Test
@DisplayName("红包适用商家列表-商卡二期-SKYX实验组：红包类型标签-优先级（非会员默认排序）")
...
//        更新数据库，将user_label表数据label_id设置为1,使其出新人首单标签35
    ...
    @DisplayName("红包适用商家列表-商卡二期-SKYX实验组：红包类型标签-优先级（非会员默认排序）")
     void couponGodDsicount(ShopListRequestDTO shopListRequestDTO) {
         ...
         // 顺序：新人、神券、店铺红包、会员红包
         assertThat( tags).isEqualTo(new ArrayList<Integer>(List.of(35, 41, 42, 42, 32) ));
