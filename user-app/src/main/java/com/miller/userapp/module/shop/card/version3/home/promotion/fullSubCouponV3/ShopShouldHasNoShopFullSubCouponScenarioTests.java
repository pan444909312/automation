 ...
 */
@EnvTag.Test
@Scenario(scenarioID = "01J46NM6T46BDDJFHRFM4RN9YX", scenarioName = "普通店铺配送商卡-SKYX01_优惠标签_已领满减红包_首页-商卡二期：已领满减红包38-不展示",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 10)
@DisplayName("普通店铺配送商卡-SKYX01_优惠标签_已领满减红包_首页-商卡二期：已领满减红包38-不展示 ")
public class ShopShouldHasNoShopFullSubCouponScenarioTests {
   ...
   @ParameterizedTest
   @DisplayName("普通店铺配送商卡_优惠标签_已领满减红包_首页-商卡二期：已领满减红包38-不展示 ")
   void shouldShowPandLeagueFullSubCouponLabel(ShopListRequestDTO shopListRequestDTO) {
      ...
      boolean result = shopIndexVO.getShopPromoteList().stream().
              noneMatch(item -> item.getType().equals(ShopPromoteEnum.USER_PACKET_AMOUNT.getType()));
