...
public class CanadaRuleTaxServiceImpl implements TaxService {
    ...
    public TaxPriceDTO computeTaxPrice(CalculateOrderBasicData ov, OrderCountInfoEx countInfo) {
        ...
        productObjectList.forEach(o -> {
            if (StringUtils.isNotBlank(o.getTaxRate()) && !"0".equals(o.getTaxRate())) {
                ...
                //（商品售价+（（配送费 - 商家承担满减 +  平台服务费 - 会员平台服务费减免） * 商品售价占比))*商品消费税率  即  （商品售价+商品按比例分摊到的配送费-商品按比例分摊的满减）* 商品消费税
                int amount = new BigDecimal(o.getPrice() + (tempDeliveryFee - ov.getFullSubMerchant() + countInfo.getPlatformFee() - countInfo.getMemberPlatformFeeDiscount()) * percent).multiply(new BigDecimal(o.getTaxRate())).divide(new BigDecimal(100), 2, RoundingMode.HALF_UP).setScale(0, RoundingMode.HALF_UP).intValue();
                taxPrice.addAndGet(amount);

                int merAmount = 0;
                if (tempMerTotal > 0) {
                    double merPercent = (double) o.getMerPrice() / tempMerTotal;
                    merAmount = new BigDecimal(o.getMerPrice() + (tempDeliveryFee - ov.getFullSubMerchant() + countInfo.getPlatformFee() - countInfo.getMemberPlatformFeeDiscount()) * merPercent).multiply(new BigDecimal(o.getTaxRate())).divide(new BigDecimal(100), 2, RoundingMode.HALF_UP).setScale(0, RoundingMode.HALF_UP).intValue();
                }
