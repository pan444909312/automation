package com.miller.userapp.module.shop.card.version2.redPacket.feature.mealsOnTime;
 ...
 */
@Scenario(scenarioID = "01JE88B2J6482ZXB6ZFYBXQX71",
        scenarioName = "商卡(中文)_普通店铺配送商卡-红包适用商家列表_营销标_标签7_出餐准时_首页-商卡二期：出餐准时 - 不展示",
...
public class ShopShouldHasNoMealsOnTimeFeature {
    ...
    @ParameterizedTest
    @DisplayName("普通店铺配送商卡-红包适用商家列表_营销标_标签7_出餐准时_首页-商卡二期：出餐准时 - 不展示")
    void shouldNotExistMealsOnTimeFeature(ShopListRequestDTO shopListRequestDTO) {
        ...
        boolean flag = shopIndexVO.getShopFeatureList().stream().noneMatch(item -> item.getType().equals(ShopFeatureTypeConstant.MEALS_ONT_IME));

        assertThat(flag).isTrue();
        assertThat(shopIndexVO.getMealsOnTime()).isEqualTo(0);
        assertThat(shopSearchMiddleEntity.getMealsOnTime().intValue()).isEqualTo(0);
