...
@Scenario(scenarioID = "01JVKR6DPY3AY792BTB6AE5DCS",
        scenarioName = "店铺的优惠信息包含店铺满减优惠",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@EnvTag.Test
@DisplayName("/api/app/user/shop/promotion")
public class UserShopPromotionHasFullDiscountPromotion {
    ...
    void testCase(ShopBaseReqDTO shopBaseReqDTO) {
        ...
        String shopNewUserPromotion =  JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 9)]").toString();
        Integer value = (Integer) JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 9)].discountInfoList[0].value");
        Integer threshold = (Integer) JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 9)].discountInfoList[0].threshold");


        assertThat(resultCode).isEqualTo(1000);
        assertThat(shopNewUserPromotion).isNotNull();
        assertThat(value).isEqualTo(200);
        assertThat(threshold).isEqualTo(1100);
