...
@EnvTag.Test
@TestFramework
@DisplayName("用户-首页资源位-中文版-banner-限制老用户可见-展示")
public class TopBannerListOnlyOldUserScenarioTests {

    @BeforeAll
    void beforeAll() {
        LoginUtils.getHeaderWithAuth(1);
    }

    @DisplayName("正常流程_获取限制老用户可见的banner")
    @Test
    public void topBannerListOnlyOldUserSuccessfully() {
        ...
        SingleModuleVo singleModuleVo = topBannerList.stream()
                .filter(banner -> banner.getIndexModuleDetailId() == Integer.parseInt(new PropertiesUtils().getProperty(this.getClass(),"user.app.index.banner.index.module.detailId.oldUser")))
                .findFirst()
                .get();
        ...
        assert singleModuleVo.getIndexModuleDetailId() == Integer.parseInt(new PropertiesUtils().getProperty(this.getClass(),"user.app.index.banner.index.module.detailId.oldUser"));
