...
public class TopBannerListGetMemberScenarioTests {
    ...
    @Test
    public void topBannerListGetMemberSuccessfully() {
        ...
        SingleModuleVo singleModuleVo = topBannerList.stream()
                .filter(banner -> banner.getIndexModuleDetailId() == Integer.parseInt(new PropertiesUtils().getProperty(this.getClass(),"user.app.index.banner.index.module.detailId.type6")))
                .findFirst()
                .get();
        ...
        assert singleModuleVo.getType() == 6;
        assert singleModuleVo.getUrl().equals(DeepLinkUrlConstant.MEMBER);
    }
}
