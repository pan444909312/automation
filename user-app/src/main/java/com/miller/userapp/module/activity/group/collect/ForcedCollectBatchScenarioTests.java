...
@EnvTag.Test
@TestFramework
@Scenario(scenarioID = "01JV6RCKS63YM5GE0Y9WC3CFP6", scenarioName = "普通天降链路-天降弹窗-智能营销对照组-手动领取-强推活动-手动领取"
        ,author = "yancancan@hungrypandagroup.com", developmentTime = 120, maintenanceTime = 0, manualTestTime = 15)
public class ForcedCollectBatchScenarioTests {
    //    普通活动红包配置
    private static final List<ForcedCollectBatchScenarioTests.RedPacketConfig> RED_PACKET_CONFIGS = Arrays.asList(
            new ForcedCollectBatchScenarioTests.RedPacketConfig(888895244L, 500, 1000)
    );
    @BeforeAll
    public static void beforeAll(){
        //        登陆指定测试账号
        PropertiesUtils propertiesUtils=new PropertiesUtils();
        String passWord =propertiesUtils.getProperty(UserLoginFlow.class,"user.app.account.activity.user02.password");
        String userName = propertiesUtils.getProperty(UserLoginFlow.class,"user.app.account.activity.user02.account");
        ...
        String userId = propertiesUtils.getProperty(UserLoginFlow.class, "user.app.account.activity.user02.userId");
...
//        删除用户领取记录以及账户里对应的神券
        for (ForcedCollectBatchScenarioTests.RedPacketConfig redPacketConfig : RED_PACKET_CONFIGS) {
...
//          // 验证用户领取状态
    ...
    private void verifyUserReceiveStatus() {
        ...
        for (ForcedCollectBatchScenarioTests.RedPacketConfig config : RED_PACKET_CONFIGS) {
    ...
    static Stream<Arguments> provideCollectData(){
        ...
        collectDetail_1.setRedPacketGroupId(2178L);
