...
@Scenario(scenarioID = "01JVKR6DPY3AY792BTB6AE5DCR",
        scenarioName = "店铺的优惠信息包含商品折扣优惠",
        author = "panjuxiang@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 5)
@EnvTag.Test
@DisplayName("/api/app/user/shop/promotion")
public class UserShopPromotionHasProductDiscountPromotion {
    ...
    void testCase(ShopBaseReqDTO shopBaseReqDTO) {
        ...
        String shopNewUserPromotion =  JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 6)]").toString();
        String label = String.valueOf(JSONPath.eval(JSON.parse(result), "$.result.shopPromotionList[?(@.promoteType == 6)].label"));



        assertThat(resultCode).isEqualTo(1000);
        assertThat(shopNewUserPromotion).isNotNull();
        assertThat(label).isLessThan("外卖低至8.8折");
