...
@EnvTag.Test
@TestFramework
@Scenario(scenarioID = "01JQTWM9TP64WNVR4FWZRCJX66", scenarioName = "APP-进入用户首页-融合App端根据红包id过滤出生效的PF红包：红包有效"
        , author = "zhangpei@hungrypandagroup.com", developmentTime = 30, maintenanceTime = 0, manualTestTime = 15)
@DisplayName("APP-根据红包id过滤出生效的PF红包")
public class QueryPFRedPacketListScenarioTests {
    ...
    @ParameterizedTest
    @DisplayName("PF融合_根据红包id过滤出生效的PF红包")
    void exchangeRedPacketSuccessfully(QueryPFRedPacketListRequestDTO requestDTO) {
        ...
        Assertions.assertThat(responseDTO.getData().getRedPacketList()).isNotNull();
