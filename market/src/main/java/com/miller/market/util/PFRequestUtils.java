...
public class PFRequestUtils {
    ...
    private static Map<String, Object> commonHeaders() {
        ...
        headers.put("latitude", PFBusinessConstant.latitude);
        headers.put("longitude", PFBusinessConstant.longitude);
        headers.put("realLatitude", PFBusinessConstant.realLatitude);
        headers.put("realLongitude", PFBusinessConstant.realLongitude);
        headers.put("zipCode", PFBusinessConstant.zipCode);
        headers.put("version", PFBusinessConstant.version);
        headers.put("pageNo", PFBusinessConstant.pageNo);
        headers.put("pageSize", PFBusinessConstant.pageSize);
        headers.put("countryCode", PFBusinessConstant.countryCode);
        headers.put("language", PFBusinessConstant.language);
        headers.put("platform", PFBusinessConstant.platform);
        headers.put("cityName", PFBusinessConstant.cityName);
        headers.put("country", PFBusinessConstant.country);
        headers.put("appTypeId", PFBusinessConstant.appTypeId);
        headers.put("authCode", PFBusinessConstant.authCode);
        headers.put("androidSafeToken", PFBusinessConstant.androidSafeToken);
        headers.put("uniqueToken", PFBusinessConstant.uniqueToken);
        headers.put("unionId", PFBusinessConstant.unionId);
        headers.put("pandaAppId", PFBusinessConstant.pandaAppId);
        headers.put("authorization", PFBusinessConstant.authorization);
        headers.put("testGroup", PFBusinessConstant.testGroup);
        return headers;
    }
