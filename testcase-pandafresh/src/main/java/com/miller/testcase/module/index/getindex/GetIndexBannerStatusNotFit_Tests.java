 ...
 */
@Scenario(
        scenarioID = "01K0E6ZB3Z1N0FXPJR0RVHKW67", // 自动生成，不要修改
        scenarioName = "pf首页-顶部banner-隐藏：未开启",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 15, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("pf首页-顶部banner-隐藏：未开启")
public class GetIndexBannerStatusNotFit_Tests {

    Map<String, Object> selectOneSql;
    @BeforeAll
     void beforeAll() throws InterruptedException {
        //查找渠道包含app的生效的非新人用户可见的优先级最高的顶部banner广告
        ...
        try {
            ...
            String sql1 = "update ad a set a.`status`=0 where a.id="+selectOneSql.get("id")+"";
            //将此广告的更新为新人可见
            FreshTestDBHelpful.executeInsertOrUpdateOrDelete(sql1);
        }catch (Exception e){
            System.out.println("无符合的顶部banner广告数据！！");
        }
    }
    @AfterAll
     void afterAll(){
        String sql2 = "update ad a set a.`status`=1 where a.id="+selectOneSql.get("id")+"";
        //将此广告更新为启用
