 ...
 */
@Scenario(
        scenarioID = "01K06KM1A67ECXKWK4CKTR2GD2", // 自动生成，不要修改
        scenarioName = "pf加购-加购失败：商品无库存",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("pf加购-加购失败：商品无库存")
public class AddShopCartNoStockGoodsFail_Tests {
    ...
    void beforeAll(){
        //查询普通下架的无库存商品
        String sql = "SELECT g.goods_id,gs.goods_sku_id FROM goods g left join goods_sku gs on gs.goods_id=g.goods_id " +
                "where g.status=0 and gs.goods_sn='YS0620001' and g.portal_id=3 and g.is_del=0 and gs.is_del=0 and type=1 limit 10";
        // 查询多条记录
