...
/**
 * getAdOfFloatWindow - channel not fit
 *
 * @author zhangpei
 * @version 2.0
 * @since 2025/08/29 10:05:00
 */
@Scenario(
        scenarioID = "01JY1A4P8K4HB3Z5J6Y1W3KQ4C", // 自动生成，不要修改
        scenarioName = "融合首页浮窗：浮窗广告-渠道不符",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("融合首页浮窗：浮窗广告-渠道不符")
public class GetAdOfFloatWindowChannelNotFit_Tests {

    Map<String, Object> selectOneSql;
    Object originalSyncWeb;

    @BeforeAll
    void beforeAll() {
        // 查找渠道包含 app 的生效的所有用户可见的优先级最高的浮窗广告
        ...
        try {
            ...
            originalSyncWeb = selectOneSql.get("sync_web");
            String sqlUpdate = "update ad_manage a set a.sync_web=1 where a.id=" + selectOneSql.get("id");
            // 将此广告更新为 sync_web=1（渠道不符：达达）
            FreshTestDBHelpful.executeInsertOrUpdateOrDelete(sqlUpdate);
        } catch (Exception e) {
            System.out.println("无符合的渠道不符浮窗广告数据！！");
        }
    }

    @AfterAll
    void afterAll() {
        if (selectOneSql != null && selectOneSql.get("id") != null && originalSyncWeb != null) {
            String sqlRevert = "update ad_manage a set a.sync_web=" + originalSyncWeb + " where a.id=" + selectOneSql.get("id");
            // 还原 sync_web
    ...
    @DisplayName("正向流程")
    @Test
    void shouldExcludeAdWhenChannelNotFit() {
