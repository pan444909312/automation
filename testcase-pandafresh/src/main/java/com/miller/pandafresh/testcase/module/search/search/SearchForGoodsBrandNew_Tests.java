 ...
 * @author zhangpei
 * @version 2.0
 * @since 2026/01/05 16:56:04
 */
@Scenario(
        scenarioID = "01K0C3VPR6T8F1HQ70SVSH30JF", // 自动生成，不要修改
        scenarioName = "搜索-搜索词匹配商品品牌名称：新索引",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 5, manualTestTime = 3)
@DisplayName("搜索-搜索词匹配商品品牌名称：新索引")
public class SearchForGoodsBrandNew_Tests {
    ...
    void shouldSuccess() {
        ...
        for (int i = 0; i < goodsList.size(); i++) {
            ...
            String sql = "SELECT * FROM goods f left join brand b on f.brand_id=b.id WHERE f.goods_id="+goodsId;
            String goodsName = FreshTestDBHelpful.executeSelectOneSql(sql).get("brand_name").toString();
            goodsNames.add(goodsName);
        }

        // 断言数组中包含指定的商品类目
        boolean containsExpectedGoods = goodsNames.contains("东方树叶");
