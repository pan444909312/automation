 ...
 */
@Scenario(
        scenarioID = "01K0C3VPR6T8F1HQ70SVSH38JW", // 自动生成，不要修改
        scenarioName = "搜索-检查商品规格:新索引",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("搜索-检查商品规格：新索引")
public class SearchResultCheckSkuNew_Tests {
    ...
    void shouldSuccess() {
        ...
        for (int i = 0; i < goodsList.size(); i++) {
            ...
            Boolean name = (Boolean) goods.get("haveSku");
            ...
            String expectName = expect.get("has_specs").toString();
            if (expectName.equals(1)){
                TestCaseHelpful.assertThatJson(name).isEqualTo(true);
            }else {
                TestCaseHelpful.assertThatJson(name).isEqualTo(false);
            }
        }
