 ...
 */
@Scenario(
        scenarioID = "01K0C3VPR6T8F1HQ70SVSH30JE", // 自动生成，不要修改
        scenarioName = "搜索-搜索词匹配商品规格名称:新索引",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 5, manualTestTime = 3)
@DisplayName("搜索-搜索词匹配商品规格名称：新索引")
public class SearchForGoodsSpecNew_Tests {
    ...
    void shouldSuccess() {
        ...
        requestBody = JSONUtils.updateJsonValue(requestBody,"keyword","500ml");
        ...
        for (int i = 0; i < goodsList.size(); i++) {
            ...
            String goodsName = FreshTestDBHelpful.executeSelectOneSql(sql).get("spec_value").toString();
            goodsNames.add(goodsName);
        }

        // 断言数组中包含指定的商品类目
        boolean containsExpectedGoods = goodsNames.contains("500ml");
