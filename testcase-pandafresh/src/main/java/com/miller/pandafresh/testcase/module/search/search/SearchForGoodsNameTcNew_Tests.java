 ...
 */
@Scenario(
        scenarioID = "01K0C3VPR6T8F1HQ70SVSH30JA", // 自动生成，不要修改
        scenarioName = "搜索-搜索词匹配商品繁体名称:新索引",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("搜索-搜索词匹配商品繁体名称：新索引")
public class SearchForGoodsNameTcNew_Tests {
    ...
    void shouldSuccess() {
        ...
        String body = "module/search/search/request/bodyTC.json";
        ...
        for (int i = 0; i < goodsList.size(); i++) {
            ...
            String goodsName = FreshTestDBHelpful.executeSelectOneSql(sql).get("goods_name_tc").toString();
            goodsNames.add(goodsName);
        }

        // 断言数组中包含指定的商品名称
        boolean containsExpectedGoods = goodsNames.stream().anyMatch(name -> name.contains("東方樹葉"));
