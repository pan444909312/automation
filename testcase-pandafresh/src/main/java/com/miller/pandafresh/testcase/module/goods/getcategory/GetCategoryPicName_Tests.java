 ...
 */
@Scenario(
            scenarioID = "01K3MSEQVZPMK6YZ3VMZMN4AES", // 自动生成，不要修改
        scenarioName = "分类页：一级分类名称、图片正确返回",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("分类页：一级分类名称、图片正确返回")
public class GetCategoryPicName_Tests {
    ...
    void shouldSuccess() {
        ...
        Map category = (Map<String, Object>) categoryList.get(0);
        String name = (String) category.get("categoryName");
        String pic = (String) category.get("groupsPic");
        int id = (int) category.get("categoryId");
        //查询一级分组信息
        String sql = "SELECT * FROM front_groups fg WHERE fg.groups_id="+id+"";
        Map<String,Object> expect = FreshTestDBHelpful.executeSelectOneSql(sql);
        String expectName = expect.get("groups_name").toString();
        String expectPic = expect.get("groups_pic").toString();
        TestCaseHelpful.assertThatJson(expectName.contains(name)).isEqualTo(true);
        TestCaseHelpful.assertThatJson(expectPic.contains(pic)).isEqualTo(true);
