 ...
 */
@Scenario(
        scenarioID = "01K0C3VPR6T8F1HQ70SVSH30JB", // 自动生成，不要修改
        scenarioName = "搜索-搜索词匹配商品英文名称：新索引",
        author = "zhangpei@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 0, manualTestTime = 3)
@DisplayName("搜索-搜索词匹配商品英文名称：新索引")
public class SearchForGoodsNameENNew_Tests {
    ...
    void shouldSuccess() {
        ...
        String body = "module/search/search/request/bodyEN.json";
        ...
        for (int i = 0; i < goodsList.size(); i++) {
            ...
            String sql = "SELECT * FROM goods f WHERE f.goods_id="+goodsId;
            String goodsName = FreshTestDBHelpful.executeSelectOneSql(sql).get("goods_name_EN").toString();
            goodsNames.add(goodsName);
        }

        // 断言数组中包含指定的商品名称
        boolean containsExpectedGoods = goodsNames.stream().anyMatch(name -> name.contains("Green"));
