 ...
 */
@Scenario(
        scenarioID = "01KEDNG43262W23GSYBG299HE6", // 需要从质量平台获取实际的 scenarioID
        scenarioName = "调度系统-调度分单-骑手接单-完单-有银行卡",
        author = "chenchunxia@hungrypandagroup.com",
        developmentTime = 180, maintenanceTime = 0, manualTestTime = 90)
@DisplayName("调度分单-骑手接单-完单-有银行卡")
public class DispatchAssignReceiveCompleteWithBankCardTests {

    @DisplayName("完整端到端流程-调度分单-骑手接单-完单-有银行卡")
    @Test
    void shouldCompleteAssignReceiveAndCompleteFlowWithBankCard() {
        // ========== 第一部分：C侧下单流程 ==========
        // 步骤1: C侧下单-用户登录
        ...
        // 步骤18: 骑手app-修改订单配送状态-签收（有银行卡）
        completeOrderWithBankCard(driverAccessToken, userAppOrderSn);
    }
     ...
     * 骑手签收订单（有银行卡）
     */
    private void completeOrderWithBankCard(String driverAccessToken, String orderSn) {
        String uri = TestcaseConfig.HOST_DELIVERY_APP + "/api/delivery/app/order/modifyDeliveryStatus";
        String method = "POST";
        Map<String, Object> headers = createDriverAppHeaders();
        headers.put("authorization", driverAccessToken);
        ...
        // 有银行卡场景：包含银行卡相关字段
        // 注意：bankCardNo 和 bankCardName 需要根据实际业务数据填写
        var requestBody = String.format("{\"driverArriveType\":11,\"operationType\":6,\"arriveRemark\":\"留言备注内容-apifox自动化测试创建，图片默认写死资源地址，免去每次上传图片到oss\",\"waitUserArrive\":0,\"orderSn\":\"%s\",\"orderCompleteImageUrlList\":[\"http://panda-auth.oss-eu-central-1.aliyuncs.com/delivery-app/170174606688616113ac9a0a74ab29cdadf98ad4cf090.jpg\"],\"orderSnList\":[\"%s\"],\"bankCardNo\":\"6217000010000000000\",\"bankCardName\":\"测试银行\"}", 
                orderSn, orderSn);
