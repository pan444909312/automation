 ...
 */
@Scenario(
        scenarioID = "01KDQQSPGMZ9HAMF7SDWK2R98P",
        scenarioName = "垫付管理-票据审核-全部tab",
        author = "chenchunxia@hungrypandagroup.com",
        developmentTime = 120, maintenanceTime = 0, manualTestTime = 30)
@DisplayName("票据审核-全部tab")
public class InvoiceAuditAllTabTests {
    ...
    @DisplayName("全部tab-列表")
    @Test
    void shouldGetAllTabList() {
        // 1) 司管登录获取 token
        ...
        String method = "POST";
        String body = String.format("{\"receiveDate\":[\"%s\",\"%s\"],\"pageNo\":1,\"pageSize\":10,\"auditStatus\":null,\"receiveDateStart\":\"%s\",\"receiveDateEnd\":\"%s\"}",
    ...
    @DisplayName("全部tab-列表-筛选城市")
    @Test
    void shouldGetAllTabListWithCity() {
        // 1) 司管登录获取 token
        String token = erpLogin();
        Map<String, Object> headers = createHeaders(token);
        ...
        String uri = TestcaseConfig.HOST_ERP + "/api/deliveryAdmin/insteadOrder/list";
        String method = "POST";
        String body = String.format("{\"receiveDate\":[\"%s\",\"%s\"],\"city\":\"杭州市\",\"pageNo\":1,\"pageSize\":10,\"auditStatus\":null,\"receiveDateStart\":\"%s\",\"receiveDateEnd\":\"%s\"}",
                lastWeekStart, lastWeekEnd, lastWeekStart, lastWeekEnd);
        var responseBody = TestCaseHelpful.sendRequest(method, uri, null, headers, body);
