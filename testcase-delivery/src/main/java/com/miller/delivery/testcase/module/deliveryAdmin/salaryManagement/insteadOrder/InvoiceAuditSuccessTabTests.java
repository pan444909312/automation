 ...
 */
@Scenario(
        scenarioID = "01KDT1P3VYAE8A4151CV0XW3A1",
        scenarioName = "垫付管理-票据审核-审核成功",
        author = "chenchunxia@hungrypandagroup.com",
        developmentTime = 120, maintenanceTime = 0, manualTestTime = 30)
@DisplayName("票据审核-审核成功tab")
public class InvoiceAuditSuccessTabTests {
    ...
    @DisplayName("审核成功列表")
    @Test
    void shouldGetSuccessAuditList() {
        // 1) 司管登录获取 token
    ...
    @DisplayName("审核成功列表-筛选城市")
    @Test
    void shouldGetSuccessAuditListWithCity() {
        // 1) 司管登录获取 token
        String token = erpLogin();
        Map<String, Object> headers = createHeaders(token);
