...
@Scenario(
        scenarioID = "01KDT14BYT0EAS11M3P6YGYYVV",
        scenarioName = "垫付管理-垫付金额管理-列表",
        author = "chenchunxia@hungrypandagroup.com",
        developmentTime = 120, maintenanceTime = 0, manualTestTime = 30)
@DisplayName("垫付金额管理-列表")
public class InsteadOrderBillListTests {
    ...
    @DisplayName("垫付金额管理-列表-搜索城市")
    @Test
    void shouldGetInsteadOrderBillListByCity() {
        // 1) 司管登录获取 token
        ...
        // 3) 垫付金额管理-列表-搜索城市
        String uri = TestcaseConfig.HOST_ERP + "/api/deliveryAdmin/insteadOrder/insteadOrderBillList";
        String method = "POST";
    ...
    @DisplayName("垫付金额管理-列表-搜索骑手")
    @Test
    void shouldGetInsteadOrderBillListByDriver() {
        // 1) 司管登录获取 token
        String token = erpLogin();
        Map<String, Object> headers = createHeaders(token);
        ...
        String uri = TestcaseConfig.HOST_ERP + "/api/deliveryAdmin/insteadOrder/insteadOrderBillList";
        String method = "POST";
        String body = String.format("{\"date\":[\"%s\",\"%s\"],\"driverId\":\"1398721719\",\"city\":\"杭州市\",\"dateStart\":\"%s\",\"dateEnd\":\"%s\",\"pageNo\":1,\"pageSize\":10}",
                lastWeekStart, lastWeekEnd, lastWeekStart, lastWeekEnd);
        var responseBody = TestCaseHelpful.sendRequest(method, uri, null, headers, body);
