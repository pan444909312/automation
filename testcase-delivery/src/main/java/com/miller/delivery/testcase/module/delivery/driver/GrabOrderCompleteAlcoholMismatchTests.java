...
@Scenario(
        ...
        scenarioName = "骑手抢单-完单流程(烟酒信息核验不一致调度送达）",
        author = "TestingConsultant@hungrypandagroup.com",
...
public class GrabOrderCompleteAlcoholMismatchTests {
    ...
    @DisplayName("骑手抢单-完单流程(烟酒信息核验不一致调度送达）完整流程")
    @Test
     ...
     * 烟酒信息核验
     */
    private void verifyAlcoholInfo(String driverAccessToken, String orderSn, boolean isMatch) {
        String uri = TestcaseConfig.HOST_DELIVERY_APP + "/api/delivery/app/order/verifyAlcoholInfo";
        String method = "POST";
        Map<String, Object> headers = createIOSDriverAppHeaders();
        headers.put("authorization", driverAccessToken);
        String body = String.format("{\"orderSn\":\"%s\",\"isMatch\":%s}", orderSn, isMatch);
        var responseBody = TestCaseHelpful.sendRequest(method, uri, null, headers, body);
