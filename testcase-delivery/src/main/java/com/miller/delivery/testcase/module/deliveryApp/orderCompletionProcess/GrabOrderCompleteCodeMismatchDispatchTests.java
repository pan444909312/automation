...
/**
 * 主干流程-骑手抢单-完单【收餐码不匹配调度送达】
 *
 * Apifox: docs/d-apifox/todo/骑手抢单-完单流程(收餐码不匹配调度送达） .apifox-cli.json
 */
@Scenario(
        scenarioID = "01K1WSWEBENDZ6580542MF4MA8",
        scenarioName = "主干流程-骑手抢单-完单【收餐码不匹配调度送达】",
        author = "TestingConsultant@hungrypandagroup.com",
        developmentTime = 240, maintenanceTime = 0, manualTestTime = 35)
@DisplayName("骑手抢单-完单流程(收餐码不匹配调度送达）")
public class GrabOrderCompleteCodeMismatchDispatchTests {

    @DisplayName("主干流程-收餐码不匹配->调度送达（按Apifox步骤）")
    @Test
    void shouldCompleteGrabOrderCodeMismatchDispatchFlow() {
        ...
        // 9) 调度完成送达（handleType=1）
        dispatchFoodDeliveryCodeCompleteMethod(siGuanToken, userAppOrderSn, 1);
    ...
    private void foodDeliveryCodeAbnormal(String driverAccessToken, String orderSn) {
        ...
        String body = String.format("{\"driverDeliveryType\":1,\"orderSn\":\"%s\",\"deliveryPic\":\"http://panda-auth.oss-eu-central-1.aliyuncs.com/delivery-app/17543861134510ea6f23935ba48098c92e38f09a0987e.jpg\",\"reasonType\":0}", orderSn);
        var responseBody = TestCaseHelpful.sendRequest("POST", uri, null, headers, body);
        TestCaseHelpful.assertThatJson(responseBody).node("resultCode").isEqualTo(1000);
        TestCaseHelpful.assertThatJson(responseBody).node("reason").isEqualTo("成功");
        TestCaseHelpful.assertThatJson(responseBody).node("success").isEqualTo(true);
    ...
    private void dispatchFoodDeliveryCodeCompleteMethod(String siGuanToken, String orderSn, int handleType) {
        String uri = TestcaseConfig.HOST_ERP + "/api/dispatch/order/foodDeliveryCodecompleteMethod";
        Map<String, Object> headers = createErpHeaders();
        headers.put("token", siGuanToken);

        String body = String.format("{\"orderSn\":\"%s\",\"handleType\":%d}", orderSn, handleType);
        var responseBody = TestCaseHelpful.sendRequest("POST", uri, null, headers, body);
        TestCaseHelpful.assertThatJson(responseBody).node("message").isEqualTo("成功");
    }
