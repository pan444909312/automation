...
@Scenario(
        ...
        scenarioName = "骑手取餐-完单流程 (联系不到用户送达)",
        author = "TestingConsultant@hungrypandagroup.com",
...
public class PickupCompleteContactFailTests {
    ...
    @DisplayName("骑手取餐-完单流程 (联系不到用户送达) 完整流程")
    @Test
    void shouldCompletePickupContactFailFlow() {
        // ========== 第一部分：C侧下单流程 ==========
        ...
        // 联系不到用户
        contactUserFail(driverAccessToken, userAppOrderSn);
        
        // 订单完成送达
        completeOrderContactFail(driverAccessToken, userAppOrderSn);
    }
     ...
     * 联系不到用户
     */
    private void contactUserFail(String driverAccessToken, String orderSn) {
        String uri = TestcaseConfig.HOST_DELIVERY_APP + "/api/delivery/app/order/orderWaitUser";
        String method = "POST";
        Map<String, Object> headers = createIOSDriverAppHeaders();
        headers.put("authorization", driverAccessToken);
        String body = String.format("{\"orderSn\":\"%s\",\"recordType\":1}", orderSn);
        var responseBody = TestCaseHelpful.sendRequest(method, uri, null, headers, body);
        TestCaseHelpful.assertThatJson(responseBody).node("resultCode").isEqualTo(1000);
    }
     ...
     * 联系不到用户送达
     */
    private void completeOrderContactFail(String driverAccessToken, String orderSn) {
        String uri = TestcaseConfig.HOST_DELIVERY_APP + "/api/delivery/app/order/modifyDeliveryStatus";
        String method = "POST";
        Map<String, Object> headers = createIOSDriverAppHeaders();
        headers.put("authorization", driverAccessToken);
        String body = String.format("{\"driverArriveType\":1,\"operationType\":6,\"orderCompleteImageUrlList\":[]," +
                "\"arriveRemark\":\"联系不到用户\",\"orderSnList\":[\"%s\"]}", orderSn);
        var responseBody = TestCaseHelpful.sendRequest(method, uri, null, headers, body);
        TestCaseHelpful.assertThatJson(responseBody).node("success").isEqualTo(true);
        TestCaseHelpful.assertThatJson(responseBody).node("reason").isEqualTo("成功");
    }
