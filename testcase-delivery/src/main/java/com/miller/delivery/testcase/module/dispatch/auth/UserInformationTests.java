package com.miller.delivery.testcase.module.dispatch.auth;
 ...
 */
@Scenario(
        scenarioID = "01JPPES6ERZ8PXCHYRFBF0E4FX", // 自动生成，不要修改
        scenarioName = "调度系统-获取基础信息-获取登录用户信息",
        author = "chenchunxia@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 20, maintenanceTime = 0, manualTestTime = 10)
@DisplayName("获取当前登录人信息")
public class UserInformationTests {
    // TestcaseConfig.HOST_ERP 是接口的请求域名（调度系统使用ERP域名）。 后面的 + "是接口的请求路径"
    String uri = TestcaseConfig.HOST_ERP + "/api/dispatch/auth/information";
    // 接口请求方式。如： GET、POST、PUT、DELETE
    String method = "POST";
    // 请求参数。如果没有传 null 即可（params = null）。比如 POST 请求通常没有 params 参数
    String params = null;
    // 断言。默认从resources目录下读取文件。下面的代码表示从 resource 的 module/xxx/response/assert_full_field.json 读取文件内容作为断言
    String assertFullField = "module/dispatch/auth/response/assert_full_field.json";
    ...
    void shouldSuccess() {
        ...
        Map<String, Object> headers = TestCaseHelpful.getHeaders("module/dispatch/auth/request/headers.json");
        // 设置请求头参数
        ...
        var requestBody = TestCaseHelpful.getJsonRequestBody("module/dispatch/auth/request/body.json");
