 ...
 */
@Scenario(
        scenarioID = "01K18DNXVFMB1VYV68WXQ65EX0",
        scenarioName = "骑手app-保底薪资列表",
        author = "chenchunxia@hungrypandagroup.com",
        developmentTime = 30, maintenanceTime = 0, manualTestTime = 10)
@DisplayName("保底薪资列表")
public class GuaranteeSalaryListTests {

    @DisplayName("保底薪资列表")
    @Test
    void shouldGetGuaranteeSalaryList() {
        // 1) 骑手登录获取 token
        String driverAccessToken = TestCaseHelpful.deliveryLogin("13300010015", "Test1234");

        // 2) 保底薪资列表
        String uri = TestcaseConfig.HOST_DELIVERY_APP + "/api/delivery/app/driverSalary/guarantee/list";
        String method = "POST";
        Map<String, Object> headers = createDriverAppHeaders();
        headers.put("authorization", driverAccessToken);
        
        // 生成日期（使用未来日期，因为JSON中使用的是2025-07-28）
