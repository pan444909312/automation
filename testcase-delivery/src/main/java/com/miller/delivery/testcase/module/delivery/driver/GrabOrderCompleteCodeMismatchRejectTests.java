...
public class GrabOrderCompleteCodeMismatchRejectTests {
    ...
    @Test
    void shouldCompleteGrabOrderCodeMismatchRejectFlow() {
        // 1) C侧下单
        ...
        // 9) 异常case：调度拒绝送达（handleType=0）
        dispatchFoodDeliveryCodeCompleteMethod(siGuanToken, userAppOrderSn, 0);

        // 10) 再次提交调度（同一订单）
        foodDeliveryCodeAbnormal(driverAccessToken, userAppOrderSn);

        // 11) 调度同意送达（handleType=1）
