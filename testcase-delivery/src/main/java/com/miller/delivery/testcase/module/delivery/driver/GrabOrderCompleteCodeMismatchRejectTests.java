...
@Scenario(
        ...
        scenarioName = "骑手抢单-完单流程(收餐码不匹配调度拒绝送达）",
        author = "TestingConsultant@hungrypandagroup.com",
...
public class GrabOrderCompleteCodeMismatchRejectTests {
    ...
    @DisplayName("骑手抢单-完单流程(收餐码不匹配调度拒绝送达）完整流程")
    @Test
     ...
     * 调度拒绝送达
     */
    private void dispatchRejectDelivery(String siGuanToken, String orderSn) {
        String uri = TestcaseConfig.HOST_ERP + "/api/dispatch/dispatch/rejectDelivery";
        String method = "POST";
        Map<String, Object> headers = createErpHeaders();
        headers.put("token", siGuanToken);
        String body = String.format("{\"orderSn\":\"%s\",\"reason\":\"收餐码不匹配\"}", orderSn);
        var responseBody = TestCaseHelpful.sendRequest(method, uri, null, headers, body);
        TestCaseHelpful.assertThatJson(responseBody).node("code").isEqualTo(1);
    }
