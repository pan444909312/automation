 ...
 */
@Scenario(
        scenarioID = "01JPPM9F3GCWT8335ZCGVWQDYZ",
        scenarioName = "调度系统-骑手管控-开启AUTO",
...
public class ControlRelieveTests {
    ...
    void shouldRelieveControl() {
        ...
        // 步骤3: 设置请求体
        var requestBody = TestCaseHelpful.getJsonRequestBody("module/dispatch/control/controlRelieve/request/body.json");
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.punishAction", 5);
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.relieveRemark", "自动化测试开启AUTO");
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.driverId", 1398714150);

        // 步骤4: 发起请求
        String uri = TestcaseConfig.HOST_ERP + "/api/dispatch/control/relieve";
        String method = "POST";
