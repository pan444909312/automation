package com.miller.testcase.module.delivery.capacityScheduling.capacityScheduling;
 ...
 */
@Scenario(
        scenarioID = "01KE620QXR096V247KEPKS8JKS", // 自动生成，不要修改
        scenarioName = "未登录-预约排班-详情",
        author = "TestingConsultant@hungrypandagroup.com", // 配置本机 Git email 后可自动生成
        developmentTime = 10, maintenanceTime = 0, manualTestTime = 5)
@DisplayName("未登录-预约排班-详情")
public class SchedulingDetailInfoNoAuthTests {
    ...
    String assertFullField = "module/delivery/capacityScheduling/response/assert_no_auth.json";
    ...
    void shouldFailWithoutAuth() {
        ...
        // 更新请求体中的动态参数
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.schedulingType", 0);
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.areaId", 51);
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.schedulingDate", "2026-1-5");
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.startTime", "22:30");
        requestBody = TestCaseHelpful.updateJsonValue(requestBody, "$.endTime", "24:00");
